...
<form class="table-filter mb-4 form-inline graph_form" action="{{route('filter.primary.drivers')}}" method="post" >
@csrf
...
@if(auth()->user())
@if(auth()->user()->role==2)
<a href="{{url(Config::get('constants.admin').'/graph-primary-drivers')}}" class="btn btn-warning btn-sm ">Reset</a>
@else
<a href="{{url(Config::get('constants.user').'/graph-primary-drivers')}}" class="btn btn-warning btn-sm">Reset</a>
@endif
@else
<a href="#">
@endif
