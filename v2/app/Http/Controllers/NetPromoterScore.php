...
{
    public function nps_score_factor_count($request=false)
    {
	...
	if($selected_department){		
		...
		->where(function($Detractors) use ($survey_id){   
if($survey_id){       
$Detractors->where('survey_answered.survey_id','=',$survey_id);                
}
})	
	...
	$Neutral= SurveyAnswered::select('survey_persons.*','survey_answered.rating as rating','survey_answered.rating as answer','survey_answered.ticket_status as ticket_status','survey_answered.updated_at as last_action_date','surveys.title as survey_title','survey_answered.person_id')
		...
		->where(function($Neutral) use ($from_date,$to_date){	
		if($from_date &&  $to_date){		
		$Neutral->whereDate('survey_answered.created_at', '>=', "$from_date 00:00:00");
		$Neutral->whereDate('survey_answered.created_at', '<=',"$to_date 23:59:59");
		}		
		})

	->where(function($Neutral) use ($selected_department){	
	if($selected_department){		
		$Neutral->where('survey_answered.department_name_id', $selected_department);
	}		
	})
	
	->where(function($Neutral) use ($survey_id){   
if($survey_id){       
$Neutral->where('survey_answered.survey_id','=',$survey_id);                
}
})	
...
->where(function($Promoters) use ($survey_id){   
if($survey_id){       
$Promoters->where('survey_answered.survey_id','=',$survey_id);                
}
})	
...
->where(function($Promoters_lastweek) use ($selected_department){	
if($selected_department){		
$Promoters_lastweek->where('survey_answered.department_name_id', $selected_department);
}		
})



->where(function($Promoters_lastweek) use ($survey_id){   
if($survey_id){       
$Promoters_lastweek->where('survey_answered.survey_id','=',$survey_id);                
}
})	
...
->where(function($Neutral_lastweek) use ($selected_department){	
if($selected_department){		
$Neutral_lastweek->where('survey_answered.department_name_id', $selected_department);
}		
})

	->where(function($Neutral_lastweek) use ($survey_id){   
	if($survey_id){       
	$Neutral_lastweek->where('survey_answered.survey_id','=',$survey_id);                
	}
	})
		->get();
...
->where(function($Detractors_lastweek) use ($selected_department){	
if($selected_department){		
$Detractors_lastweek->where('survey_answered.department_name_id', $selected_department);
}		
})

->where(function($Detractors_lastweek) use ($survey_id){   
if($survey_id){       
$Detractors_lastweek->where('survey_answered.survey_id','=',$survey_id);                
}
})		
...
try{
	...
	return view('frontend.survey.rating',compact('page','Questions'));
...
try{
	...
	$Questions=Questions::select('questions.organization_id as qorgid','questions.id as qid','questions.survey_id as qsurvey_id','questions.label as qlabel','questions.sublabel as qsublabel','questions.input_type as qinput_type')->where('sequence_order',1)->where('survey_id',Session::get('survey_id'))->get();		 
	foreach ($Questions as $key => $value) {
	$Questions[$key]->qoptions = QuestionOptions::select('question_options.option_value as qpvalue','question_options.id as qoptionid')
	->where('question_id', $value->qid)
	->orderBy('id','asc')
...
public function takepostrating(Request $request)
{
	...
	try{
		...
		else
		{
			...
			return redirect('takevoiceconversation')->with('info', 'Please take voice message..'); 
...
public function post_choosedepartment(Request $request){
...
if(is_array($request->first_questin_range))
	{
		...
		$delete_exist_answered=SurveyAnswered::where('person_id',Session::get('person_id'))->whereNotIn('department_name_id',[0])->delete();
	
		foreach($request->first_questin_range as $key=>$value){	
		
		
	if(isset($request->other_department) && !empty($request->other_department) ){
	$departments = Departments::where('department_name','LIKE',"%other%")->get()->first();
	
	
	if(isset($departments->id) && !empty($departments->id)){
		if($value==$departments->id){
			$other_department_info=$request->other_department??'';
			
		}
		else{
			$other_department_info="";
		}
	}
	}
	else{
		...
		$first_questionans=SurveyAnswered::insert([
            [
                ...
                "answeredby_person"=>$other_department_info??'',
                "person_id"=>Session::get('person_id')??0,
				"logged_user_id"=>auth()->user()->id??Session::get('logged_user_id'),
				'created_at' =>Carbon::now(),
				'updated_at' =>Carbon::now(),
...
'logged_user_id' =>auth()->user()->id??Session::get('logged_user_id')??0,
	...
	return redirect('feedback_confirmation')->with('success', 'Feedback survey have been complete..');
...
public function feedbackconfirmation(Request $request){
	
	try{
$departments = Departments::orderBy('sorting','asc')->orderBy('department_name','asc')->get();

if(Session::get('rating')<=8){			
$this->send_ticket_status_emails("opened",Session::get('person_id'));			
}

return view('frontend.survey.feedbackconfirmation',compact('departments'));
...
public function take_voice_messaage(Request $request)
{		
...
public function send_ticket_opened_mail($person_id=null){
	...
	if($person_id){		
        ...
        ->join('departments', 'survey_answered.department_name_id', '=', 'departments.id')
   
        ->where('survey_answered.person_id',$person_id)
        ->get(['survey_answered.*','departments.department_name as option_value']);	
