...
*/
		...
		if($request->question_id) {                    
			...
			->where(function($Passives) use ($role){
				...
				$user_mapped_departments=Departments_Users::where('user_id',auth()->user()->id??0)->get()->pluck('department_id');									
				//$q_departments=QuestionOptions::whereIn('department_id',$user_mapped_departments)->get()->pluck('id');
				$Passives->whereIn('survey_answered.department_name_id',$user_mapped_departments??0);				
			}			
			else if($role==4){				
			...
			if($role==2){
				...
				$user_mapped_departments=Departments_Users::where('user_id',auth()->user()->id??0)->get()->pluck('department_id');									
				//$q_departments=QuestionOptions::whereIn('department_id',$user_mapped_departments)->get()->pluck('id');			
				$Promoters->whereIn('survey_answered.department_name_id',$user_mapped_departments??0);				
			}			
			else if($role==4){				
	...
	try{
		...
		$user_mapped_departments=Departments_Users::where('user_id',auth()->user()->id??0)->get()->pluck('department_id');									
		$departments=Departments::whereIn('id',$user_mapped_departments??0)->get();
       ...
       ->join('departments', 'departments.id', '=', 'survey_answered.department_name_id')
		...
		->where(function($person_responses_data) use ($role,$user_mapped_departments){
			if($role==3){
			$person_responses_data->whereIn('survey_answered.department_name_id',$user_mapped_departments);
			}
		})
		
        
        ->get(['survey_answered.*','departments.department_name as option_value','survey_persons.ticker_final_number','survey_persons.assigned_ticket']);
        // $person_responses_data=SurveyAnswered::join('survey_persons', 'survey_answered.person_id', '=', 'survey_persons.id')
...
$user_mapped_departments=Departments_Users::where('user_id',auth()->user()->id??0)
->join('survey_answered', 'survey_answered.department_name_id', '=', 'mapping_depatemnts_to_users.department_id')
...
$rating_of_dep_activities=DB::table('rating_of_dep_activities')->where('person_id',$person_id??0)
->where(function($rating_of_dep_activities) use ($role,$user_mapped_departments){
if($role==3){
$rating_of_dep_activities->whereIn('rating_of_dep_activities.department_id',$user_mapped_departments);
}
})	
->get('activity_id')->pluck('activity_id')->toarray();



        return view('admin.responses.responses_view', compact('pageTitle','person_responses_data','person_data','person_responses_status_data','voice_message','departments','rating_of_dep_activities'))
...
$remove_exist_person=DB::table('rating_of_dep_activities')->where('person_id',$request->id??0)->delete();
...
$allselected_activities=implode(",",$fvalue);
$Ticket_Status=SurveyAnswered::where('person_id', $request->id)->where('department_name_id',$fkey)
	->update(
	[  
		"department_activities"=>$allselected_activities,
	...
	[
	'person_id' =>$request->id??0, 
	'survey_id' =>$request->survey_id??0,
	'created_at' =>Carbon::now(),
	'updated_at' =>Carbon::now(),
	'activity_name' =>$value??'',
	'activity_id' =>$key??'',
	'department_id' =>$fkey??0,
	'department_name' =>'',
...
->where(function($Ticket_Status) use ($role,$user_mapped_departments){
if($role==3){
$Ticket_Status->whereIn('survey_answered.department_name_id',$user_mapped_departments);
}
})
->update(
[  
"department_activities"=>'',
	...
	$remove_exist_person=DB::table('rating_of_dep_activities')->where('person_id',$request->id??0)
	->where(function($remove_exist_person) use ($role,$user_mapped_departments){
	if($role==3){
	$remove_exist_person->whereIn('rating_of_dep_activities.department_id',$user_mapped_departments);
	}
	})
...
public function transferred_ticket($request=false){
	...
	return redirect()->back()->with('error', "Work in progress."); 
try{
