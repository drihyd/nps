...
<input type="hidden" name="rating" value="{{$person_data->rating??''}}">
@foreach($departments as $key=>$value)

<label ><strong>{{$value->department_name??''}}</strong></label>	

<div class="form-group" id="survey_id">

@foreach($value->activities as $akey=>$avalue)
<div class="mt-3">
<input   id="option_checkboxes" type="checkbox" name="first_activities[{{$value->id??''}}][{{$avalue->activityid??''}}]" class="btn btn-scale btn-scale-asc-2" value="{{$avalue->activity_name}}" 


@if(in_array($avalue->activityid,$rating_of_dep_activities??0))
