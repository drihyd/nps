...
<form class="table-filter mb-4 form-inline graph_form" action="{{route('filter.feedback.composition')}}" method="post" >
@csrf
...
@if(auth()->user())
@if(auth()->user()->role==2)
<a href="{{url(Config::get('constants.admin').'/feedback-composition')}}" class="btn btn-warning btn-sm ">Reset</a>
@else
<a href="{{url(Config::get('constants.user').'/feedback-composition')}}" class="btn btn-warning btn-sm">Reset</a>
@endif
@else
<a href="#">
@endif
